...
export class MeasureDetailPanelComponent implements OnInit, OnDestroy {
  ...
  private _setDefaults(): void {
    ...
    if (this.scalesOptions.length) this.selectedScale = this.scalesOptions[0];
  }
  ...
  ngOnInit(): void {
    ...
    this.stateSubscription = this.annotationToolsService.measurePanelDetailState$.subscribe(state => {
      ...
      if (
        state.type === MARKUP_TYPES.MEASURE.LENGTH.type ||
    ...
    this.measurePanelService.scaleState$.subscribe(state => {
      if (state.created) {
        ...
        if (this.measureData.dimtext === '0.0') {
          ...
          if (this.measureData.hasScale === false) this.selectedScale = this.scalesOptions.find(item => item.label === state.scaleLabel);
        }
      }
  ...
  selectCurrentScale(markup?) {
    if (!this.scalesOptions.length) return;
    //element scale
  ...
  setMeasurementOnLength(markup: any) {
    this.measureData = markup;
    this.measureData.angleRadians = this.measureData?.getangleofline(
      this.measureData.x,
      this.measureData.y,
      this.measureData.w,
      this.measureData.h,
