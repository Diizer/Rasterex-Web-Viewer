...
export class InsertModalComponent implements OnInit {
  ...
  convertArrayToString(array) {
    return array.map(range => (range.length === 2 ? `${range[0] + 1}-${range[1] + 1}` : `${range[0] + 1}`)).join(',');
  }
  ...
  importPages() {
    ...
    this.loadingStatus = 'LOADING';
    RXCore.importPages(this.file, newPageRange, checkedPages, this.visible === 'REPLACE', this.selectedCount())
      .then(() => {
        this.loadingStatus = 'NONE';
        this.visible = 'NONE';
      })
